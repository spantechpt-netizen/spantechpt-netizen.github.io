...
هذا المستند موجّه لمهندس السيرفر. كل ما يحتاجه لتشغيل النظام موجود هنا.
...
النسخ يتم بأمر `sqlite3`، لأن نسخ الملف بـ `cp` أثناء عمل النظام قد يلتقط قاعدة
ناقصة (النظام يستخدم وضع WAL):
...
```cron
# نسخة يومية 2:00 صباحاً، مع الاحتفاظ بآخر 30 يوماً
0 2 * * * sqlite3 /opt/spantech-crm/data/spantech.db ".backup '/backups/spantech-$(date +\%F).db'" && find /backups -name 'spantech-*.db' -mtime +30 -delete
```

النسخ آمن أثناء عمل النظام — لا حاجة لإيقافه. عدّل المسار حسب مكان التنصيب
الفعلي، ويُفضّل نقل النسخ إلى تخزين خارج السيرفر أيضاً.
...
- [ ] مهمة النسخ الاحتياطي اليومية تعمل، وتم اختبار استرجاع نسخة
